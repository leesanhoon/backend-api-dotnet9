...
public interface ILidService
{
    ...
    Task<CreateOrUpdateLidResult> CreateAsync(CreateLidRequest request, CancellationToken cancellationToken);
...
public sealed record LidResponse(int Id, string Name, string? Description, int CategoryId, string CategoryName, IReadOnlyList<LidPriceResponse> Prices);
